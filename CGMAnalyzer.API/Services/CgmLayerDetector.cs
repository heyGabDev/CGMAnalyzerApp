namespace CGMAnalyzer.API.Services
{
    public class CGMLayerDetector : ICGMLayerDetector
    {
        public List<int> DetectLayers(string tempPath)
        {
            ...
            byte[] layerStartPattern = [0x20, 0x04];// Motif supposé pour début de couche (ex: BEGIN PICTURE en CGM binaire peut être 0x20 0x04)
            return FindPatternOffsets(cgmData, layerStartPattern);
